...
def calculate_radius(AVG_DISTANCE):
...
def create_obj_data_new(objects, satellite_data, AVG_DISTANCE):

	largest_x_coor_satellite = find_largest_x_coor_satellite(satellite_data)
	
	obj_data = []
	for i in range(objects):
		radius = calculate_radius(AVG_DISTANCE)
		theta = random.random()*180 - (90)
		theta_rad = math.radians(theta)
		x_coor = 100*random.random()
		while x_coor < largest_x_coor_satellite:
			x_coor = radius * math.cos(theta_rad)
		y_coor = radius * math.sin(theta_rad)
		obj = {
			'x_coor' : x_coor,
			'y_coor' : y_coor,
			'label' : "O" + str(i+1),
...
def create_obj_data_angle(objects, satellite_data, angle):
...
def max_flow_with_edmonds_karp_with_priorities(G, no_of_satellites, obj_data):
	...
	print("avg_no_of_objects_detected", avg_no_of_objects_detected)
	return avg_no_of_objects_detected[-1]
